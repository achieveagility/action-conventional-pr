...
if (issuePrefix !== "") {
    ...
    if (validMatch) {
        ...
        fail(`Issue suffix is invalid. Expected '${issuePrefix}<positive-integer>' (for example ${issuePrefix}123).`);
    }
}
...
if (!allowedVerbs.has(firstWord)) {
    fail("PR subject must start with an allowed imperative verb (for example: add, update, fix, remove, refactor).");
}
export {};
